 ...
 */
export default function PackageActiveToggleButton({
  packageId,
  packageLabel,
  isActive,
  variant = "button",
  ...
  const currentlyActive = isPackageActiveFlag(isActive);
  const label = currentlyActive ? "利用停止" : "利用再開";
  ...
  async function handleClick() {
    ...
    const { error: updateError } = await supabase
      .from("packages")
      .update({
        is_active: toPackageActiveDbValue(!currentlyActive),
      })
      .eq("id", packageId);

    if (updateError) {
      setError(
        `${packageLabel}の更新に失敗しました：${updateError.message}`
      );
      setBusy(false);
      return;
    }
  ...
  return (
    <div
      className={
        variant === "menuitem" ? "" : "inline-flex flex-col items-start gap-1"
      }
    >
      <button
        type="button"
