...
import {
  AuthConfigError,
  deriveSupplierPurchasePriceBulkRequestId,
  isUuid,
  sessionActorKey,
...
import { createSupplierPurchasePrices } from "@/lib/supplierPurchasePrices/createSupplierPurchasePrices";
import {
  toSafeSupplierPurchasePricesError,
  toSafeSupplierPurchasePricesSuccess,
} from "@/lib/supplierPurchasePrices/safeSupplierPurchasePricesDto";

export const runtime = "nodejs";

/**
 * 仕入先起点の仕入価格一括登録（PRODUCT のみ）。
 * create_supplier_purchase_prices RPC を service_role でのみ実行。
 */
export async function POST(request: NextRequest) {
  ...
  if (originResult !== "ok") {
    ...
    gatewayLog({
      route: "supplier-purchase-price-bulks",
  ...
  try {
    requestId = deriveSupplierPurchasePriceBulkRequestId(sessionActorKey(session), idempotencyKey);
  } catch (e) {
  ...
  const result = await createSupplierPurchasePrices(requestId, rest);
