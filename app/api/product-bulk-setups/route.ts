...
import {
  AuthConfigError,
  deriveProductBulkSetupRequestId,
  isUuid,
  sessionActorKey,
...
import { createProductBulkSetup } from "@/lib/productBulkSetup/createProductBulkSetup";
import {
  toSafeProductBulkSetupError,
  toSafeProductBulkSetupSuccess,
} from "@/lib/productBulkSetup/safeProductBulkSetupDto";

export const runtime = "nodejs";

/**
 * 商品一括登録（products のみ。価格非同梱）。
 * create_product_bulk_setup RPC を service_role でのみ実行。
 */
export async function POST(request: NextRequest) {
  ...
  try {
    requestId = deriveProductBulkSetupRequestId(
  ...
  const result = await createProductBulkSetup(requestId, rest);
    ...
    const status =
      result.error_code === "NOT_FOUND"
        ...
        : result.error_code === "CONFIG_ERROR"
          ? 503
          : result.error_code === "DUPLICATE_PRODUCT" ||
