...
import { createProductSetup } from "@/lib/productSetup/createProductSetup";
import {
 ...
 * - request_id はサーバー派生（body の request_id は信用しない）
 * - create_product_setup RPC を service_role でのみ実行
 */
export async function POST(request: NextRequest) {
  ...
  try {
    requestId = deriveProductSetupRequestId(sessionActorKey(session), idempotencyKey);
  } catch (e) {
  ...
  const { request_id: _ignoredRequestId, ...rest } = input;
  void _ignoredRequestId;

  const result = await createProductSetup(requestId, rest);
