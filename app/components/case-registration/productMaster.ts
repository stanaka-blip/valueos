...
export async function fetchActiveProducts(): Promise<{
  ...
  try {
    // products.is_active は環境によって boolean / 文字列の差があるため、
    // 取得後にアプリ側で有効判定する（マスタ取得失敗を避ける）。
    const { data, error } = await supabase
      .from("products")
      .select("id, name, model_no, manufacturer_id, is_active")
      .order("name", { ascending: true });
    ...
    return {
      data: (data || [])
        .filter((item) => {
          const active = item.is_active as boolean | string | null;
