...
assert.ok(caseDetail.includes("粗利 v1"));
assert.ok(caseDetail.includes("確定粗利（請求・発注ベース）"));
assert.ok(caseDetail.includes("見込粗利（参考）"));
assert.ok(caseDetail.includes("computeConfirmedCaseProfit"));
assert.ok(caseDetail.includes("価格未設定を含む"));
assert.ok(!caseDetail.includes("その他支払（暫定0）"));
assert.ok(!caseDetail.includes("売上（入金合計）"));
assert.ok(!caseDetail.includes("const other = 0"));
