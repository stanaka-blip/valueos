...
BEGIN
  ...
  BEGIN
    ...
    -- 第1パス: 検証のみ（価格必須にはしない。supplier は第2パスで検証）
    ...
    -- 第2パス: 明細保存（payload の supplier/単価を snapshot。未指定は NULL）
    FOR v_idx IN 0 .. jsonb_array_length(v_lines) - 1 LOOP
      v_line := v_lines->v_idx;
      ...
      -- supplier / prices from payload (UI resolved or manual). NULL allowed.
      v_supplier_id := NULL;
      IF NULLIF(btrim(COALESCE(v_line->>'supplier_id', '')), '') IS NOT NULL THEN
        BEGIN
          v_supplier_id := btrim(v_line->>'supplier_id')::uuid;
        EXCEPTION WHEN OTHERS THEN
      ...
      IF NULLIF(btrim(COALESCE(v_line->>'purchase_price', '')), '') IS NOT NULL THEN
        BEGIN
          v_unit_purchase := (v_line->>'purchase_price')::numeric;
        EXCEPTION WHEN OTHERS THEN
          RAISE EXCEPTION 'APP:INVALID_INPUT:仕入単価が正しくありません';
        END;
        IF v_unit_purchase < 0 THEN
          RAISE EXCEPTION 'APP:INVALID_INPUT:仕入単価が正しくありません';
        END IF;
      END IF;

      v_unit_sales := NULL;
      IF NULLIF(btrim(COALESCE(v_line->>'sales_price', '')), '') IS NOT NULL THEN
        BEGIN
          v_unit_sales := (v_line->>'sales_price')::numeric;
        EXCEPTION WHEN OTHERS THEN
          RAISE EXCEPTION 'APP:INVALID_INPUT:販売単価が正しくありません';
        END;
        IF v_unit_sales < 0 THEN
          RAISE EXCEPTION 'APP:INVALID_INPUT:販売単価が正しくありません';
        END IF;
      END IF;

      BEGIN
        v_is_manual_price := coalesce((v_line->>'is_manual_price')::boolean, false);
      EXCEPTION WHEN OTHERS THEN
        v_is_manual_price := false;
      END;

      IF v_unit_purchase IS NOT NULL OR v_unit_sales IS NOT NULL THEN
        v_price_fetched_at := clock_timestamp();
      ELSE
        v_price_fetched_at := NULL;
      END IF;

      IF v_unit_purchase IS NOT NULL AND v_unit_sales IS NOT NULL THEN
        v_gross := round(v_unit_sales * v_quantity) - round(v_unit_purchase * v_quantity);
      ELSE
        v_gross := NULL;
      END IF;
        ...
        INSERT INTO public.case_products (
          ...
          CASE WHEN v_unit_sales IS NULL THEN NULL ELSE round(v_unit_sales * v_quantity) END,
          CASE WHEN v_unit_purchase IS NULL THEN NULL ELSE round(v_unit_purchase * v_quantity) END,
          v_gross,
          NULL, NULL, v_is_manual_price, v_price_fetched_at, v_memo
        );
