-- 発注編集: orders ヘッダ更新 + order_items 置換 + order_amount 再計算を 1 関数（= 1 トランザクション）で行う。
-- 途中失敗は plpgsql ブロックの EXCEPTION で全ROLLBACK（ヘッダ・明細とも元に戻る）。
-- 新テーブルなし。GRANT は現行の orders / order_items 直書きと同じ authenticated/anon。
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    ELSIF v_app_message IS NULL THEN
      v_app_message := '発注の更新に失敗しました';
    END IF;
    -- このブロック内の UPDATE/DELETE/INSERT はサブトランザクションごと ROLLBACK される。
    RETURN jsonb_build_object(
      'ok', false,
...
COMMENT ON FUNCTION public.replace_purchase_order(jsonb) IS
  '発注ヘッダ更新と明細置換を同一トランザクションで行う。失敗時は全ROLLBACK。パッケージAMT/COMP行は削除不可。COMP金額は0。';
