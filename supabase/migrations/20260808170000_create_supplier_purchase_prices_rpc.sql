-- Ver1.0: 仕入先起点の仕入価格一括登録 RPC（create_supplier_purchase_prices）
--
-- 目的:
-- - 1 supplier_id に対し、複数 PRODUCT の purchase_prices を 1 トランザクションで追加
-- - 既存価格行の UPDATE/DELETE / auto end_date は行わない（INSERT のみ）
-- - #106 / #108 RPC は変更しない
--
-- 範囲:
-- - 新規テーブル: supplier_purchase_price_bulk_requests
-- - 新規関数: public.create_supplier_purchase_prices(jsonb)
-- - EXECUTE / ledger: service_role のみ
-- - PACKAGE 非対応
-- - Additive only

CREATE TABLE IF NOT EXISTS public.supplier_purchase_price_bulk_requests (
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'supplier_purchase_price_bulk_requests_status_check'
      AND conrelid = 'public.supplier_purchase_price_bulk_requests'::regclass
  ) THEN
    IF NOT EXISTS (
      SELECT 1
      FROM pg_constraint
      ...
      RAISE EXCEPTION
        'supplier_purchase_price_bulk_requests_status_check exists with unexpected definition';
    END IF;
  ELSE
    ALTER TABLE public.supplier_purchase_price_bulk_requests
      ADD CONSTRAINT supplier_purchase_price_bulk_requests_status_check
      CHECK (status IN ('PROCESSING', 'COMPLETED', 'FAILED'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS supplier_purchase_price_bulk_requests_supplier_id_idx
  ON public.supplier_purchase_price_bulk_requests (supplier_id);

CREATE INDEX IF NOT EXISTS supplier_purchase_price_bulk_requests_status_created_at_idx
  ON public.supplier_purchase_price_bulk_requests (status, created_at);

COMMENT ON TABLE public.supplier_purchase_price_bulk_requests IS
  '仕入先起点仕入価格一括登録RPCの冪等キー。';

ALTER TABLE public.supplier_purchase_price_bulk_requests DISABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.supplier_purchase_price_bulk_requests FROM PUBLIC;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    REVOKE ALL ON TABLE public.supplier_purchase_price_bulk_requests FROM anon;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    REVOKE ALL ON TABLE public.supplier_purchase_price_bulk_requests FROM authenticated;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    REVOKE ALL ON TABLE public.supplier_purchase_price_bulk_requests FROM service_role;
    GRANT SELECT, INSERT, UPDATE
      ON TABLE public.supplier_purchase_price_bulk_requests TO service_role;
  END IF;
END $$;
...
COMMENT ON FUNCTION public.create_supplier_purchase_prices(jsonb) IS
  '仕入先1社に対する複数PRODUCT仕入価格を1トランザクションでINSERT。既存行非更新。冪等ledger付き。';
